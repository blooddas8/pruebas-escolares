...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $conn->prepare("INSERT INTO clientes (nombre, apellido, direccion, telefono, email) VALUES (?, ?, ?, ?, ?)");
    ...
    if ($stmt->execute()) {
        ...
        header("Location: ../V vista/agregar_cliente.html?success=1");
        exit; // Asegúrate de usar exit después de header
    } else {
        // Redirigir después de un error
        header("Location: ../V vista/agregar_cliente.html?error=" . urlencode($stmt->error));
        exit; // Asegúrate de usar exit después de header
    }
